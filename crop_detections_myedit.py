...
def main(detections_json_path: str,
    ...
    images_failed_dload_crop, num_downloads, num_crops = download_and_crop(
        ...
        detections_crops_df_path=detections_crops_df_path,
...
def download_and_crop(
    ...
    for img_path in tqdm(sorted(detections.keys())):
        ...
        future = pool.submit(
            load_and_crop, img_path, detections_crops_df_path, images_dir, container_client, bbox_dicts,
...
def load_and_crop(img_path: str,
    ...
    num_new_crops = 0
     ...
     ## save crops to df
    #if my edit doesn't work then comment out these lines 
    #if os.path.exists(detections_crops_df_dir):
        #crop_df = pd.read_csv(detections_crops_df_dir,index_col=0)
    #else:
    ...
    bboxes_cat_tocrop: Dict[str, str] = {}
    bboxes_conf_tocrop: Dict[str, float] = {}
    for i, bbox_dict in enumerate(bbox_dicts):
        ...
        if not os.path.exists(crop_path) or (
            ...
            bboxes_cat_tocrop[crop_path] = bbox_dict['category']
            bboxes_conf_tocrop[crop_path] = bbox_dict['conf']
    ...
    for crop_path, bbox in bboxes_tocrop.items():
        image_crop_dict = {'full_img_dir':img_path,
                           'img_id':img_path.split('/')[-1].replace('.JPG','')}
        image_crop_dict['crop_path'] = crop_path
        image_crop_dict['bbox_norm'] = bbox
        image_crop_dict['category'] = bboxes_cat_tocrop[crop_path]
        image_crop_dict['md_conf'] = bboxes_conf_tocrop[crop_path]

        num_new_crops += save_crop(
            img, bbox_norm=bbox, square_crop=square_crops, save=crop_path)
        
        # append crop details to dataframe
        df = pd.DataFrame.from_dict(image_crop_dict,orient='index').transpose()
        df = df[sorted(df.columns)]
        df.to_csv(detections_crops_df_dir, mode='a', 
                                             header=not os.path.exists(detections_crops_df_dir))
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--detections_crops_df_path',
        help='path to store dataframe per chunk of detected/cropped data')
...
if __name__ == '__main__':
    ...
    detections_crops_df_path=args.detections_crops_df_path,
